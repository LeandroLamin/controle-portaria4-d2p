...
async function acsPesquisar() {
    ...
    if (cpfBusca) {
        ...
        const ultimoP02 = await dbBuscar(TABELA_ACS, { cpf: cpfBusca, data_gte: hoje, data_lte: hoje }, { order: 'id.desc', limit: 1 });
        const jaEntrou = ultimoP02 && ultimoP02.length > 0 && ultimoP02[0].acesso === 'ENTRADA';
        if (jaEntrou) {
...
function _acsRenderizarTabela(lista) {
    ...
    lista.forEach(item => {
        ...
        [
            ...
            [item.veiculo || '', false], [item.carreta || '', false],
